...
export function EmergencyCard({ name, emoji, description, active, onClick }: EmergencyCardProps) {
  return (
    <DashboardCard
      onClick={onClick}
      hoverable={true}
      className={`border transition-all duration-300 ${
        active
          ? 'bg-gradient-to-br from-rose-950/20 via-slate-900/40 to-amber-950/20 border-rose-500/50 shadow-[0_0_20px_rgba(239,68,68,0.15)] scale-[1.01]'
    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#0a0a0f] border border-white/10 text-3xl shadow-inner transition-transform duration-300 ${
          active ? 'scale-105 border-rose-500/30' : ''
        }`}>
